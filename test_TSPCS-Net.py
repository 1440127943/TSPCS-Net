...
class TSPCSFrame():
    ...
    def test_one_img_from_path_1(self, path):
        ...
        mask, mask02, mask03, mask04  = self.net.forward(img5)
...
def test_TSPCS_Net():
    source = './dataset/pavement/test/images/'
    val = os.listdir(source)
    disc = 1
    solver = TSPCSFrame(TSPCS_Net_)
    solver.load('weights/TSPCS-Netpavement_plus_spatial_multi.th')
    tic = time()
    target = 'test_results/'

    if not os.path.exists(target):
        os.mkdir(target)
    gt_root = './dataset/pavement/test/1st_manual'
    total_m1 = 0
    ...
    for i, name in enumerate(val):
        image_path = os.path.join(source, name.split('.')[0] + '.JPG')
        print(image_path)
        ...
        if i == 0:
            total_time1 = time_end - time_start
        ...
        ground_truth_path = os.path.join(gt_root, name.split('.')[0] + '.png')
        ...
        acc, sen, F1_score, IoU = accuracy(predi_mask[:, :, 0], gt)
        ...
        print(i + 1, acc, sen, F1_score, IoU, calculate_auc_test(new_mask / 8., ground_truth), single_fps)
    ...
    print(np.mean(total_F1_score), np.std(total_F1_score))
    print(np.mean(total_IoU), np.std(total_IoU))
    print(np.mean(total_auc), np.std(total_auc))

    fps = i / (total_time - total_time1)
    print(' ({:.2f} fps total_time:{:.2f})'.format(fps, (total_time - total_time1)))
