...
def default_pavement_loader(img_path, mask_path):
    img = cv2.imread(img_path)
    img = cv2.resize(img, (448, 448))
...
def read_pavement_datasets(root_path, mode='train'):
    images = []
    masks = []
...
class ImageFolder(data.Dataset):
    ...
    def __init__(self,root_path, datasets='pavement',  mode='train'):
        self.root = root_path
        self.mode = mode
        self.dataset = datasets
        self.images, self.labels = read_pavement_datasets(self.root, self.mode)

    def __getitem__(self, index):

        img, mask = default_pavement_loader(self.images[index], self.labels[index])
